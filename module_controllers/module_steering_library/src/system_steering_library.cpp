...
using ControllerTwistReferenceMsg =
  platform_steering_library::SystemSteeringLibrary::ControllerTwistReferenceMsg;
...
{
SystemSteeringLibrary::SystemSteeringLibrary()
: controller_interface::ChainableControllerInterface()
{
}

controller_interface::CallbackReturn SystemSteeringLibrary::on_init()
{
  try
  {
    param_listener_ = std::make_shared<platform_steering_library::ParamListener>(get_node());
...
controller_interface::CallbackReturn SystemSteeringLibrary::set_interface_numbers(
...
controller_interface::CallbackReturn SystemSteeringLibrary::on_configure(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  if (params_.use_stamped_vel)
  {
    ref_subscriber_twist_ = get_node()->create_subscription<ControllerTwistReferenceMsg>(
      ...
      std::bind(&SystemSteeringLibrary::reference_callback, this, std::placeholders::_1));
  }
  else
  {
    ref_subscriber_unstamped_ = get_node()->create_subscription<geometry_msgs::msg::Twist>(
      ...
      std::bind(
        &SystemSteeringLibrary::reference_callback_unstamped, this, std::placeholders::_1));
  }
...
void SystemSteeringLibrary::reference_callback(
  const std::shared_ptr<ControllerTwistReferenceMsg> msg)
{
...
void SystemSteeringLibrary::reference_callback_unstamped(
...
SystemSteeringLibrary::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration
SystemSteeringLibrary::state_interface_configuration() const
{
...
SystemSteeringLibrary::on_export_reference_interfaces()
{
...
bool SystemSteeringLibrary::on_set_chained_mode(bool chained_mode)
{
  // Always accept switch to/from chained mode
  return true || chained_mode;
}

controller_interface::CallbackReturn SystemSteeringLibrary::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
controller_interface::CallbackReturn SystemSteeringLibrary::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
controller_interface::return_type SystemSteeringLibrary::update_reference_from_subscribers()
{
...
controller_interface::return_type SystemSteeringLibrary::update_and_write_commands(
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  // если контроллер не находится в зависимости от других, то тк на референс интерфейс
  // текущего контроллера не будет приходить данные по референс интерфейсу и следовательно
  // , по логике, их нужно забрать с топика, который сделали real-time))))
  // ну или , по логике, если контроллер первый в списке контроллеров, то ему следовало бы 
  // забрать инфу о управляющих воздействиях (скорости) с топика и записать их на референс
  // интерфейс, дабы последующие контроллеры могли с ним работать (однако, возникает вопрос,
  // играет ли текущий флаг chained_mode, на последующие контроллеры в цепи. Выяснять это, у 
  // меня пока нет времени, но я обязательно вернусь к этому вопросу).
