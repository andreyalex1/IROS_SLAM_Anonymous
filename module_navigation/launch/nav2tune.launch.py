...
ARGUMENTS = [
    ...
    DeclareLaunchArgument('params_file', default_value=PathJoinSubstitution([
                                                        get_package_share_directory('platform_navigation'),
                                                        'config',
                                                        'nav2.yaml'
                                                        ]),
...
def launch_setup(context, *args, **kwargs):
    ...
    realsense2_launch_path = os.path.join(get_package_share_directory('platform_camera_2'), 'launch', 'platform_navcam_launch.py')
    realsense2 = IncludeLaunchDescription(PythonLaunchDescriptionSource(realsense2_launch_path))

    localization_launch_path = os.path.join(get_package_share_directory('platform_localization'), 'launch', 'localization.launch.py')
    localization = IncludeLaunchDescription(PythonLaunchDescriptionSource(localization_launch_path),
                                            launch_arguments={'pose_estimator': "'odometry'"}.items())
    ...
    inverse_cmd = Node(
        package="platform_inverse",
        executable="platform_inverse",
        output="log",
        parameters=[],
        remappings=[],
