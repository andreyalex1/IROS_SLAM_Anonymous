...
{
  SystemOdometry::SystemOdometry()
  : rclcpp::Node("platform_odometry"),
    current_roll(0.0),
    current_pitch(0.0),
    current_yaw(0.0),
    ...
    angular_acc_(velocity_rolling_window_size_)
  {
    ...
    joint_state_sub = create_subscription<sensor_msgs::msg::JointState>(
      ...
      std::bind(&SystemOdometry::joint_state_subscriber_callback, this, std::placeholders::_1));

    imu_subscriber = create_subscription<sensor_msgs::msg::Imu>(
      imu_sub_topic,
      rclcpp::SensorDataQoS(),
      std::bind(&SystemOdometry::imu_subscriber_callback, this, std::placeholders::_1));

    odometry.set_wheel_params(wheel_radius, wheel_base, wheel_track);
    odometry.init(this->now());
  }

  void SystemOdometry::imu_subscriber_callback(const sensor_msgs::msg::Imu::SharedPtr msg)
  {
  ...
  void SystemOdometry::joint_state_subscriber_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    ...
    {
      steer_angle_deg_accumulated = msg->position[0] + msg->position[1] - msg->position[2] - msg->position[3]; 
      ...
      wheel_angular_velocity_accumulated =  -msg->velocity[0] - msg->velocity[2] + msg->velocity[1] + msg->velocity[3];

      wheel_average_angular_velocity = wheel_angular_velocity_accumulated / joint_count;

      robot_linear_velocity  = wheel_average_angular_velocity * (1 - measure_error) * wheel_radius * M_PI / 180;
    ...
    double dt = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - last_time_point).count() / 1000.0;
